...
void main() {
  ...
  group('AppSettings with writable initialize', () {
    ...
    test('sets data normally with valid readOnly and writable json maps', () async {
      //
      for (final readOnly in validReadOnlyMaps) {
        for (final writable in validWritableMaps) {
          ...
          expect(
            ...
            readOnly['test_setting_1'],
          );
          expect(
            AppSettings.getSetting('test_setting_2', onError: ((err) => null)),
            readOnly['test_setting_2'],
          );
          expect(
            AppSettings.getSetting('test_setting_3', onError: ((err) => null)),
            readOnly['test_setting_3'],
          );
           expect(
            AppSettings.getSetting('test_writable_setting_1', onError: ((err) => null)),
            writable['test_writable_setting_1'],
          );
          expect(
            AppSettings.getSetting('test_writable_setting_2', onError: ((err) => null)),
            writable['test_writable_setting_2'],
          );
          expect(
            AppSettings.getSetting('test_writable_setting_3', onError: ((err) => null)),
            writable['test_writable_setting_3'],
          );
        }
      }
    });
